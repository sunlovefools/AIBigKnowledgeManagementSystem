...
from app.service.rag.agentic_modification.nodes import search_and_group_node
from app.service.rag.agentic_modification.services import vector_search
...
def _anchors_from_env() -> tuple[list[str], list[str]]:
    ...
    Expected format:
    - AGENTIC_MODIFICATION_NODE2_LEXICAL='["$50","minimum order amount"]'
    - AGENTIC_MODIFICATION_NODE2_SEMANTIC='["wholesale minimum order"]'
    """
    lexical_raw = os.getenv("AGENTIC_MODIFICATION_NODE2_LEXICAL", "[\"$50\"]")
    semantic_raw = os.getenv(
        "AGENTIC_MODIFICATION_NODE2_SEMANTIC",
    ...
    if not isinstance(lexical, list) or not isinstance(semantic, list):
        raise AssertionError(
            "AGENTIC_MODIFICATION_NODE2_LEXICAL and AGENTIC_MODIFICATION_NODE2_SEMANTIC must be JSON arrays."
        )
...
def test_node2_real_backend_search_with_fake_anchors_from_env(monkeypatch):
    ...
    Enable with:
      $env:AGENTIC_MODIFICATION_NODE2_INTEGRATION='1'
      $env:AGENTIC_MODIFICATION_NODE2_LEXICAL='["$50","minimum order amount","wholesale customers"]'
      $env:AGENTIC_MODIFICATION_NODE2_SEMANTIC='["wholesale customer minimum order","minimum order requirement for wholesale"]'
      python -m pytest backend/tests/test_agentic_modification_search_group_node.py -k real_backend -q
    """
    if os.getenv("AGENTIC_MODIFICATION_NODE2_INTEGRATION", "").strip() != "1":
